...
namespace ORB_SLAM2 {
    ...
    void LocalMapping::SetMap(Map *map) {
        mpMap = map;
    }

    void LocalMapping::save(boost::archive::binary_oarchive &ar, const unsigned int version) const {
        ar & mbMonocular & mbResetRequested & mbFinishRequested & mbFinished;
        auto vmlNewKeyFrames = createIdList(mlNewKeyFrames);
        ar & vmlNewKeyFrames;
    ...
    void LocalMapping::load(boost::archive::binary_iarchive &ar, const unsigned int version) {
        ar & mbMonocular & mbResetRequested & mbFinishRequested & mbFinished;
        std::list<uint64_t> idList;
        ar & idList;
        mlNewKeyFrames = createObjectList<KeyFrame>(idList);
