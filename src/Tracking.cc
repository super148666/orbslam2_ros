...
namespace ORB_SLAM2 {
    ...
    void Tracking::SetMap(Map *map) {
        mpMap = map;
    }

    void Tracking::save(boost::archive::binary_oarchive &ar, const unsigned int version) const {
        ar & mState & mLastProcessedState & mSensor
        & mvIniLastMatches & mvIniMatches & mvbPrevMatched & mvIniP3D;
        ar & mbOnlyTracking & mbVO;

        int _nfeatures = mpORBextractorLeft->GetNFeatures();
        int _nlevels = mpORBextractorLeft->GetLevels();
        int _iniThFAST = mpORBextractorLeft->GetIniThFAST();
        int _minThFAST = mpORBextractorLeft->GetMinThFAST();
        float _scaleFactor = mpORBextractorLeft->GetScaleFactor();
        ar & _nfeatures & _scaleFactor & _nlevels & _iniThFAST & _minThFAST;

        auto mpReferenceKF_id = mpReferenceKF->mnId;
        ar & mpReferenceKF_id;
        ar & mK & mDistCoef & mbf & mMinFrames & mMaxFrames & mThDepth & mDepthMapFactor & mnMatchesInliers;
        auto mpLastKeyFrame_id = mpLastKeyFrame->mnId;
        ar & mpLastKeyFrame_id;
        ar & mnLastKeyFrameId & mnLastRelocFrameId;
        ar & mVelocity & mbRGB;
        auto vmlpTemporalPoints = createIdList(mlpTemporalPoints);
        ar & vmlpTemporalPoints;

        ar & mlRelativeFramePoses;
        auto vmlpReferences = createIdList(mlpReferences);
        ar & vmlpReferences;
        ar & mlFrameTimes;
        ar & mlbLost;
    ...
    void Tracking::load(boost::archive::binary_iarchive &ar, const unsigned int version) {
        ar & mState & mLastProcessedState & mSensor
        & mvIniLastMatches & mvIniMatches & mvbPrevMatched & mvIniP3D;
        ar & mbOnlyTracking;
        ar & mbVO;
        int _nfeatures, _nlevels, _iniThFAST, _minThFAST;
        float _scaleFactor;
        ar & _nfeatures & _scaleFactor & _nlevels & _iniThFAST & _minThFAST;
        mpORBextractorLeft = new ORBextractor{_nfeatures, _scaleFactor, _nlevels, _iniThFAST, _minThFAST};
        ...
        ar & mK & mDistCoef & mbf & mMinFrames & mMaxFrames & mThDepth & mDepthMapFactor & mnMatchesInliers;
        ar & id;
        mpLastKeyFrame = KeyFrame::objectListLookup[id];
        ar & mnLastKeyFrameId & mnLastRelocFrameId;
        ar & mVelocity & mbRGB;
        std::list<uint64_t> idList;
        ar & idList;
        mlpTemporalPoints = createObjectList<MapPoint>(idList);

        ar & mlRelativeFramePoses;
        std::list<uint64_t> vmlpReferences;
        ar & vmlpReferences;
        mlpReferences = createObjectList<KeyFrame>(vmlpReferences);
        ar & mlFrameTimes;
        ar & mlbLost;
    }
    ...
    void Tracking::SetKeyFrameDatabase(KeyFrameDatabase *kfd) {
        mpKeyFrameDB = kfd;
    }

    void Tracking::SetORBVocabulary(ORBVocabulary *voc) {
        mpORBVocabulary = voc;
    }
